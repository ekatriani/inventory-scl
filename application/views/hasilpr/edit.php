    ...
    <div class="col-md-8">
        <div class="card shadow-sm border-bottom-primary">
            ...
            <div class="card-body">
                ...
                <?= form_open_multipart('', [], ['jmljadi' => 0, 'id_barang' => $barang['id_barang']]); ?>
                ...
                <div class="row form-group">
                    <label class="col-md-3 text-md-right" for="prduksi_id">Produksi</label>
                    <div class="col-md-9">
                        <div class="input-group">
                            <select name="prduksi_id" id="prduksi_id" class="custom-select">
                                <option value="" selected disabled>Pilih Produksi</option>
                                <?php foreach ($produksi as $pr) : ?>
                                    <option <?= $hasilpr['prduksi_id'] == $pr['id_produksi'] ? 'selected' : ''; ?> <?= set_select('prduksi_id', $pr['id_produksi']) ?> value="<?= $pr['id_produksi'] ?>"><?= $pr['nama_produksi'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <?= form_error('prduksi_id', '<small class="text-danger">', '</small>'); ?>
                    </div>
                </div>
                <div class="row form-group">
                ...
                <div class="row form-group">
                 ...
                 <div class="row form-group">
                    <label class="col-md-3 text-md-right" for="supplier_id">Customer</label>
                    <div class="col-md-9">
                        <div class="input-group">
                            <select name="supplier_id" id="supplier_id" class="custom-select">
                                <option value="" selected disabled>Pilih Customer</option>
                                <?php foreach ($supplier as $s) : ?>
                                    <option <?= $barang['supplier_id'] == $s['id_supplier'] ? 'selected' : ''; ?> <?= set_select('supplier_id', $s['id_supplier']) ?> value="<?= $s['id_supplier'] ?>"><?= $s['nama_supplier'] ?></option>
                                <?php endforeach; ?>
                            </select>
                            <div class="input-group-append">
